...
class DataValidation:
    ...
    def validate_dataset_schema(self) -> bool:
        try:
            ...
            if len(column_list)== len(list(train.columns)) and len(column_list)==len(list(test.columns)):
                if column_list==list(train.columns) and column_list==list(test.columns):
    ...
    def get_and_save_data_drift_report(self):
        try:
            profile = Profile(sections=[DataDriftProfileSection()])  #create data drift profile section

            train_df, test_df = self.get_train_and_test_df()

            profile.calculate(train_df, test_df)      #Calculate statistics

            report = json.loads(profile.json())    #get calculation in json->str format convert it into python object list or dic
